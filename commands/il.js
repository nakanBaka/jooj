...
exports.run = async (client, message, args, tools) => {
    ...
    arraySort(invites, 'usos', { reverse: true }); // Be sure to enable 'reverse'

    // Next, we need to go through each invite on the server, to format it for a table
    let possibleInvites = [['Usuarios', 'Usos']]; // Each array object is a rown in the array, we can start up by setting the header as 'User' & 'Uses'
    ...
    const embed = new Discord.MessageEmbed()
        .setColor(0xCB5A5E)
        .addField('rank', `\`\`\`${table.table(possibleInvites)}\`\`\``); // This will be the field holding the leaderboard
